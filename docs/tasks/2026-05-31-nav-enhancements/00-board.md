# 📋 Task Board — 내비게이션 3종 (T5)

> teammate / 기존 repo 확장 / feature-based

## 범위 (사용자 지시)
1. **상세뷰 일자 이동**: `/attendance/[date]`에 이전/다음 버튼 → 어제/내일 날짜 상세로 이동.
2. **뒤로가기**: 상세뷰에 뒤로 버튼 → 달력(`/attendance`) 복귀.
3. **월 선택**: 달력 상단 날짜(월 라벨) 클릭 → 월 선택 picker로 직접 월 이동(T4의 ‹ › 화살표와 병존).
