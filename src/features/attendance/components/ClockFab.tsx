...
function ClockFabInner({ onRegistered }: { onRegistered?: () => void }) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={handleClick}
      className="fixed bottom-28 right-4 z-40 mx-auto rounded-full bg-coral px-6 py-4 font-bold text-white shadow-lg disabled:bg-foreground/20 disabled:text-white/70 sm:right-[calc(50vw-18rem)]"
    >
      {label}
    </button>
  );
